...
export function LoreBook() {
  ...
  const [currentCategory, setCurrentCategory] = useState<"World" | "Character" | "Quest" | "Achievement">("World")
  ...
  const filteredEntries = loreEntries.filter((entry) => entry.category === currentCategory && entry.discovered)
  ...
  return (
    <Card className={`${getLoreBookClass()} overflow-hidden`}>
      ...
      <CardFooter className="flex justify-between">
        ...
        <Button variant="outline" size="sm" onClick={nextPage} disabled={currentPage >= filteredEntries.length - 1}>
          Next
